...
def _safe_filename_component(s: str) -> str:
    """
    Keep it simple: replace path separators and strip whitespace.
    """
...
def _run_one_condition(
    *,
    ...
    """
    Worker-safe: loads its own model from disk (avoids pickling large cobra models).
    Writes a single parquet per condition:
      outdir/condition_id=XXXX.parquet
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    logging.info("Running batch: n_conditions=%d, n_targets=%d, n_jobs=%d, backend=%s", len(rows), len(targets), args.n_jobs, args.backend)

    # Avoid oversubscription on shared servers unless user explicitly controls it.
