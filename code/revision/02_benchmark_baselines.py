...
def main() -> None:
    ...
    md = ["# Baseline benchmarking — summary",
          ...
          f"- **C** Curated widths — paper-aligned {len(curated)}-reaction width__ panel (ours)",
          "",
          "## Headline numbers",
          "",
          "```",
          ...
          ("- Inputs-only baselines establish a floor on severity regression "
           ...
           "(RandomForest / XGBoost ≈ 0.96), reflecting the strong O2_limited "
           "majority class."),
          ("- GEM-summary baselines achieve high R² (≥ 0.93). **Caveat:** the "
           "FBA objective is in this feature vector by construction "
           "(severity = obj/obj_max), so its R² ≈ 0.998 reflects "
           "target-construction overlap, not new predictive content."),
          ("- On the same curated width panel, all three learners perform "
           "competitively (macro-F1 0.957–1.000; RMSE 0.005–0.031). The "
           "diagnostic signal is therefore carried primarily by the "
           "**flexibility-based representation**, not by a uniquely optimal "
           "learner. Random forest reached macro-F1 = 1.000 on this small "
           "dataset but lacks SHAP-equivalent interpretability of the same "
           "form; XGBoost is used as the consistent explainable learner."),
          ("- **Headline message:** the revision's claim is *not* that "
           "XGBoost is uniquely optimal. The claim is that flexibility-based "
           "features carry meaningful diagnostic signal and support an "
           "interpretable diagnostic workflow across model classes."),
          ]
