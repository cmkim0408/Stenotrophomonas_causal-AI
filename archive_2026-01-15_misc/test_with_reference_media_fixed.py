#!/usr/bin/env python
"""
레퍼런스 모델과 동일한 미디어 설정으로 FBA 테스트 (수정 버전)
"""
...
def apply_media_from_tsv(model, tsv_path):
    ...
    if 'exchange_id' in df.columns:
        id_col = 'exchange_id'
    elif df.columns[0].lower().startswith('exchange'):
        id_col = df.columns[0]
    else:
        id_col = df.columns[0]
    ...
    # 컬럼 인덱스로 접근 (컬럼 이름이 숫자인 경우)
    if lb_col is None:
        lb_col = df.columns[1] if len(df.columns) > 1 else None
    if ub_col is None:
        ub_col = df.columns[2] if len(df.columns) > 2 else None
    
    if lb_col is None or ub_col is None:
        raise ValueError(f"minflux/maxflux 컬럼을 찾을 수 없습니다. 컬럼: {list(df.columns)}")
...
def test_fba(model):
    ...
    try:
        ...
        if solution.objective_value:
            print(f"목적함수 값 (성장률): {solution.objective_value:.6f}")
        else:
            print(f"목적함수 값: None")
...
def main():
    ...
    # 먼저 레퍼런스 모델로 FBA 테스트 (검증)
    print("\n" + "="*70)
    print("Step 0: 레퍼런스 모델 FBA 검증")
    print("="*70)
    
    ref_test_model = apply_media_from_tsv(cobra.io.read_sbml_model(str(ref_model_path)), media_tsv)
    ref_solution, ref_status = test_fba(ref_test_model)
    
    if ref_status == "SUCCESS":
        print(f"[OK] 레퍼런스 모델 FBA 성공: 성장률 = {ref_solution.objective_value:.6f}")
    else:
        print(f"[WARN] 레퍼런스 모델 FBA 실패: {ref_status}")
    
    # 누락된 9개 반응 추가
    print("\n" + "="*70)
    print("Step 1: 누락된 9개 반응 추가")
    print("="*70)
    ...
    if solution:
        print(f"성장률: {solution.objective_value:.6f} 1/h" if solution.objective_value else "성장률: 0.0")
